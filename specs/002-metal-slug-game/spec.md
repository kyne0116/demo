...
- **FR-009**: 游戏必须支持关卡进度保存，玩家可以随时中断和继续游戏
- **FR-010**: 游戏必须包含基础音效系统（射击、爆炸、收集音效），不需要背景音乐
...
- Q: 游戏进度保存机制 → A: 支持关卡进度保存，玩家可以随时中断和继续
- Q: 音效和音乐系统 → A: 包含基础音效（射击、爆炸、收集），不含背景音乐
